...
const AddItem = () => {
  ...
  useEffect(() => {
    if (user?.institution) {
      setForm((prev) => ({ ...prev, institute: user.institution }));
    }
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!location) {
      toast.error("Please pin a handover location on the map.");
      return;
    }

    if (!form.title || !form.description || images.length === 0) {
      toast.error("Please fill all required fields (including images)");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="p-8 space-y-8">
            ...
            <div className="space-y-6">
              ...
              {/* Upload Box (Only show if less than 4 images) */}
              {images.length < 4 && (
                <div
      ...
      ${images.length === 0 ? "border-gray-300 hover:border-blue-400" : "border-blue-300 bg-blue-50/20"}`}
                >
          ...
          {!user?.whatsapp?.isVerified && (
            <div className="mb-4 p-4 bg-blue-50 border border-blue-100 rounded-xl flex items-start gap-3 animate-pulse">
              <div className="flex-shrink-0 mt-0.5">
                <svg
                  ...
                  viewBox="0 0 24 24"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.29.174-1.413-.074-.124-.272-.198-.57-.347z" />
                </svg>
              </div>
              <div>
                ...
                <p className="text-xs text-blue-700 mt-0.5">
                  Don't forget to verify your number on your{" "}
                  <button
                    type="button"
                  ...
                  so buyers can contact you 3x faster via WhatsApp.
                </p>
              </div>
            </div>
          )}
